    ...
    if query_vector:
      if "sqlite" in str(db.bind.url):
          try:
              ...
              matched = query_chunks(query_vector, limit=limit)
              for item in matched:
                  if item["score"] < 0.3:
                      continue
                  chunk = await db.get(DocChunk, item["id"])
                  if chunk:
                      results.append({
                          "type": "doc_chunk",
              ...
              logger.warning(f"ChromaDB doc_chunks search failed: {e}")
      else:
          try:
    ...
    if query_vector:
      if "sqlite" in str(db.bind.url):
          try:
              ...
              matched = query_entities(query_vector, limit=limit)
              for item in matched:
                  if item["score"] < 0.35:
                      continue
                  entity = await db.get(Entity, item["id"])
                  if entity:
              ...
              logger.warning(f"ChromaDB entities search failed: {e}")
      else:
          try:
              stmt = (
